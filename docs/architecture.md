...
| Installable app | **PWA** — `manifest.webmanifest`, icons, the existing service worker | No new dependency. Installs into its own window from Chrome, Edge or Android, and via Share → Add to Home Screen on iOS. The worker adds only an offline page; it caches no conversation and no dashboard file, so an installed app can never show a stale inbox |
