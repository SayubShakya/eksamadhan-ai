...
| Installable app | **PWA** — `manifest.webmanifest`, generated icons and iOS launch images, one service worker | No new dependency (the asset generator uses puppeteer from outside package.json, like the draw.io tools). Installs into its own window from Chrome, Edge or Android; Share → Add to Home Screen on iOS. The worker caches the app shell and Vite's hashed assets so the app opens offline, stale-while-revalidates our own icons, and never touches `/api` or another origin — an installed app cannot show a stale inbox. `display: standalone` rather than fullscreen, so the phone's notification shade stays reachable for a tool built around alerts |
